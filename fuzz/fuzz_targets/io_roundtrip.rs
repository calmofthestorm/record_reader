...
use record_reader::{Format, IoRecordReader, IoRecordWriter, RecordReader, RecordWriter};
...
fuzz_target!(|data: &[u8]| {
    ...
    let mut w = IoRecordWriter::new(Vec::<u8>::new(), format);
    for r in &records {
    ...
    let mut rr = IoRecordReader::from_read(Cursor::new(bytes), format, max_read_size);
    match format {
